...
def find_vines_in_vinerow(entity_type, vine_row_id):
    ...
    FIWARE_ORION_BASE_URL = "http://localhost:1027/v2/entities/"

    try:
        # Query Orion to get all entities of type Vine with the specified vine_row_id
        fiware_orion_url = (
            f"{FIWARE_ORION_BASE_URL}?type={entity_type}&q=vine_row_id=={vine_row_id}"
        )
        ...
        if vine_response.status_code == 200:
            ...
            print(
                f"Failed to retrieve vine data. Status code: {
...
def find_photos_for_vines(entity_type, vine_id):
    ...
    try:
        ...
        fiware_orion_url = (
            f"{FIWARE_ORION_BASE_URL}?type={entity_type}&q=vine_id=={vine_id}"
        )
        ...
        if photo_response.status_code == 200:
            ...
            photo_url = [
                entity["photo"]["value"] for entity in photo_data if "photo" in entity
            ]
            return photo_url
        else:
            print(
