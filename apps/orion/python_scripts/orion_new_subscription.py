...
subscription_url = "http://localhost:1027/v2/subscriptions"
...
def create_subscription(entity_id, entity_type, attribute, endpoint):
    subscription_payload = {
        ...
        "subject": {
            "entities": [
                {"id": entity_id, "type": entity_type, "fiware-service": "openiot"}
            ],
            "condition": {
                "attrs": [attribute],
    ...
    response = requests.post(
        subscription_url, headers=headers, data=json.dumps(subscription_payload)
    )
    ...
    if response.status_code == 201:
        ...
        print(
            f"Failed to create subscription for {entity_id} - {
                attribute
...
for entity in entities:
    for attribute, attribute_value in entity.items():
        ...
        create_subscription(
            entity["id"], entity["type"], attribute, notification_endpoint
        )
